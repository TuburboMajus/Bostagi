...
def dologin(form):
	...
	try:
		form.update({"username":form['username'].strip()})
	except:
		return redirect(url_for("auth.login"))

	authenticator = auth_blueprint.get_configuration('authenticator')
	user = authenticator.search_user(form["username"])
	...
	if user is not None and user['user'].attributes['password'] == form.get('password'):
		if user['is_disabled']:
			...
			goto = url_for('email_manager.listEmails')
			return redirect(next_ or goto)
	else:
