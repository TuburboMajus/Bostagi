...
def install_preset_objects(credentials, admin_credentials):	
	...
	bostagi = Bostagi(version=APP_VERSION)
	...
	admin_user = User(
		id=str(uuid4()),privilege=admin_privilege['id'],username=admin_credentials['username']
	)
	admin_user['password'] = admin_credentials['password']

	MysqlEntityStorage(Bostagi,**credentials).create(bostagi)
...
def setup(app_paths, args):
	...
	domain_name = ask_for_domain_name()
	...
	template_config['app']['domain_name'] = domain_name
...
if __name__ == "__main__":
	...
	print("\n"); width = common_funcs.print_pattern("B.o.s.t.A.g.I Server"); print(); print("#"*width); print()

	parser = argparse.ArgumentParser(prog="Installs a BostAgI server")
	parser.add_argument(
		'-l', '--logging-dir', help='Directory where log files will be stored', 
		default=os.path.join("/","var","log","bostagi")
	)
	parser.add_argument(
		'-s', '--services-dir', 
		help='Directory where bostagi services files will be stored', 
	...
	if setup(app_paths, args):
		LOGGER.info("BostAgI setup completed successfully")
	else:
